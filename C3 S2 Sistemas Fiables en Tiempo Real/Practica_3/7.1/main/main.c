...
void vTaskCode3( void * pvParameters )
{
    ...
    static portMUX_TYPE lock_tarea = portMUX_INITIALIZER_UNLOCKED; // Mutex para proteger la sección crítica de la tarea 3
    spinlock_initialize(&lock_tarea); // Inicializa el spinlock
    ...
    while(1)
    {
       if (!flag_stats)
            printf("\t\t\tI\t\t\t%llu\n", esp_timer_get_time()/1000 );
        taskENTER_CRITICAL(&lock_tarea); // Evita que la tarea sea interrumpida por otras tareas, incluyendo la ISR
       //Consume CPU cycles
       for (long i = 0; i < ITERATE_3; i++) {
           __asm__ __volatile__("NOP");
       }
         taskEXIT_CRITICAL(&lock_tarea); // Permite que la tarea sea interrumpida por otras tareas, incluyendo la ISR
       if (!flag_stats)
            printf("\t\t\tO\t\t\t%llu\n", esp_timer_get_time()/1000 );
...
void app_main(void)
{
    ...
    xTaskCreate( vTaskCode3, "TASK3", STACK_SIZE, NULL, 10, &xHandle3 );  //Prioridad 1
    xTaskCreate( vTaskCode4, "TASK4", STACK_SIZE, NULL, 1, &xHandle4 );  //Prioridad 1
    xTaskCreate( vTaskCode5, "TASK5", STACK_SIZE, NULL, 1, &xHandle5 );  //Prioridad 1 --> CAMBIO: 1 -> 2
