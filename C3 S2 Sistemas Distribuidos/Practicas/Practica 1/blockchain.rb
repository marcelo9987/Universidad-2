...
def create_first_block
	...
	instance_variable_set( "@b#{i}", 
												 Block.first(
												    { from: "NULL", to: "NULL", what: "GENESIS", qty: 0 }
													)
											 )
	LEDGER << @b0
	pp @b0
...
def add_block
	i = 1
	loop do
    puts "\n¿Quieres crear un nuevo bloque? (Y/n)"
    answer = gets.chomp.downcase
    break if answer == "n"  # si el usuario dice "n", salimos del loop
  ...
  puts "\nFin de la blockchain. Se crearon #{i} bloques."
end
